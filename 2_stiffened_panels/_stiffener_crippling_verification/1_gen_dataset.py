...
total_samples = 4000
per_AR = 30
outer_loops = int(total_samples / per_AR)
for foo in range(outer_loops):
    ...
    log_SR = np.random.uniform(np.log(10.0), np.log(200.0))
    SR = np.exp(log_SR)
    ...
    for AR in AR_vec:
        ...
        if not(0.05 <= flat_plate.affine_aspect_ratio < 32.0):
            continue
