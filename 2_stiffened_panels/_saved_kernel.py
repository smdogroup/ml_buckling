...
#     0.1, 0.4, 0.4, # untrained part now
#     1.9135627407198488, 0.9932663026728726, 1.215272984407997, 1.131107170054227,
#     0.26204218577436783, 0.1, # new xi quadratic term
#     0.09904271405413381, 0.05, # new zeta quadratic term
#     0.9859641343064356
# ])
theta_opt = np.array([
    0.3, 0.596365688768736, 0.3926058918384833, 
    0.05, 0.3, 0.1, 0.1, 1.7543911463102955, 0.8334368582888703, 
    1.0598405426403348, 1.1196284417425444, 0.22474028505504423, 
    0.1, 0.08655950851675914, 0.1, 1.0
])
...
def kernel(xp, xq, theta):
    ...
    SE_kernel2 = theta[4] * np.exp(-0.5 * (d0**2 / theta[5]**2 + d2**2 / theta[6]**2))
    SE_kernel = (
        SE_factor
    ...
    # v2
    inner_kernel = BL_kernel * gamma_kernel + SE_kernel + xi_kernel + zeta_kernel + SE_kernel2
    return inner_kernel
