...
slender_name = "slender-bin-log" if use_logs else "slender-bin"
slender_folder = os.path.join(case_folder, slender_name)
if os.path.exists(slender_folder):
    shutil.rmtree(slender_folder)
if not os.path.exists(slender_folder):
    os.mkdir(slender_folder)

Dstar_name = "Dstar-bin-log" if use_logs else "Dstar-bin"
Dstar_folder = os.path.join(case_folder, Dstar_name)
...
for ibin, bin in enumerate(slender_bins):
    ...
    for iDstar, Dstar_bin in enumerate(Dstar_bins):
        ...
        plt.plot(
            ...
            kx0[mask],
            "o",
            label=r"$\xi\in" + f"[{Dstar_bin[0]:.2f},{Dstar_bin[1]:.2f}]" + r"$",
        )
...
for iDstar, Dstar_bin in enumerate(Dstar_bins):
    ...
    plt.xlabel(r"$\rho_0$")
    plt.ylabel(r"$\lambda_{min}*$")
    plt.title(r"$\xi \in " + f"[{Dstar_bin[0]:.2f},{Dstar_bin[1]:.2f}]" + r"$")
    plt.savefig(
