...
if include_quadratic:
    ...
    print(
        f"{load}_{BC}_crit norm = {w_linear[0,0]} + {w_linear[1,0]} * Dstar + {w_linear[2,0]} * Dstar**2 + {w_linear[3,0]} * (sigmoid(b/h/10)-1)"
    )
    ...
    X_linear = np.concatenate(
        (np.ones((n_train, 1)), X[:n_train, 0:1], X[:n_train, 2:3]), axis=-1
    )
    Y_linear = Y[:n_train, :]
    ...
    print(
        f"{load}_{BC}_crit norm = {w_linear[0,0]} + {w_linear[1,0]} * Dstar + {w_linear[2,0]} * (sigmoid(b/h/10)-1)"
    )
...
if include_quadratic:
    ...
    X_linear_test = np.concatenate(
        (np.ones((X_test.shape[0], 1)), X_test[:, 0:1], X_test[:, 2:3]), axis=-1
    )
...
if load == "Nxy":
    ...
    for i in range(X_test.shape[0]):
        ...
        if xi > 1.0:
            ...
            Y_pred[i, 0] = (
                4.0 / np.pi ** 2 * xi ** 0.5 * (11.7 + 0.532 * xi + 0.938 * xi ** 2)
            )
else:
