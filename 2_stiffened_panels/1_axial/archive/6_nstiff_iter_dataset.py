...
"""
Goal of this script is to try and stitch together lowest global mode at each
rho0 together into smooth curve for each gamma and see how that goes
"""
...
def axial_load(rho0, gamma, solve_buckling=True, first=False):
    ...
    for nstiff in range(1, 6+1):
        ...
        # exit nstiff loop only if global_lambda_star is not None
        # meaning it found a global mode
        if solve_buckling:
            if global_lambda_star is not None:
                break
        
        else: # if just doing closed-form, you can exit nstiff for loop
            break
    ...
    if comm.rank == 0 and solve_buckling:
        ...
        my_df.to_csv("6-nstiff-iter.csv", header=first, mode="w" if first else "a")
...
if __name__ == "__main__":
    ...
    for igamma, gamma in enumerate(gamma_vec[::-1]):
        ...
        N11_FEA = np.array(
            [
                axial_load(rho0, gamma=gamma, first=igamma == 0 and irho0 == 0)[0]
