...
for isample,sample in enumerate(samples):
    ...
    ax.plot(xvec, sample,color=colors[isample], alpha=0.5, linewidth=1)
#ax.set_axis_off(

fontsize=20

if args.case == 1:
    plt.title(r"$k(x,x\prime) = exp(-\frac{(x-x\prime)^2}{2 \cdot 1^2})$", fontsize=fontsize, pad=15)
if args.case == 2:
    plt.title(r"$k(x,x\prime) = exp(-\frac{(x-x\prime)^2}{2 \cdot 0.2^2})$", fontsize=fontsize, pad=15)
if args.case == 3:
    plt.title(r"$k(x,x\prime) = 1 + 0.2 \cdot x \cdot x\prime$", fontsize=fontsize, pad=15)
if args.case == 4:
    plt.title(r"$k(x,x\prime) = 1 + 0.2 \cdot relu(x \cdot x\prime)$", fontsize=fontsize, pad=15)
if args.case == 5:
    plt.title(r"$k(x,x\prime) = exp(-\frac{(x-x\prime)^2}{2 \cdot 0.2^2}) \cdot relu(1-|x|) \cdot relu(1 - |x\prime|) + 1.0 + 1.0 \cdot relu(-x) \cdot relu(-x\prime)$", fontsize=10, pad=15)
if args.case == 6:
    plt.title(r"$k(x,x\prime) = 1.0$", fontsize=fontsize, pad=15)
if args.case == 7:
    plt.title(r"$k(x,x\prime) = 1 + 0.2 * relu(-x) * relu(-x\prime)$", fontsize=fontsize, pad=15)
if args.case == 8:
    plt.title(r"$k(x,x\prime) = exp(-\frac{(x-x\prime)^2}{2 \cdot 0.2^2}) \cdot \tau(1-|x|) \cdot \tau(1 - |x \prime|) + 1.0 + 1.0 \cdot \tau(-x) \cdot \tau(-x\prime)$", fontsize=10, pad=15)

if args.case in [5,8]:
    plt.ylim(-7, 7)

plt.savefig(f"kernel-demo{args.case}.png", dpi=400,bbox_inches='tight', pad_inches=0.02)