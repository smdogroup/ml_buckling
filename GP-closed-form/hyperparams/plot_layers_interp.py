...
def plot_panel(ax, subdf, title):
    ...
    for nl in sorted(subdf["nlayers"].unique()):
        ...
        ax.plot(
            d["nparams"], d["intR2"],   # <-- CHANGED (interp R2)
            marker="o",
            linestyle="-",
    ...
    ax.set_xscale("log")
    ax.set_xlim(1e1, 2e5)
    ax.set_ylim(-4.0, 1.05)  # <-- CHANGED (intR2 goes below -1 in your data)

    ax.set_xlabel("Number of parameters")
    ax.set_ylabel(r"Interpolation $R^2$")  # <-- CHANGED
    ax.set_title(title)
...
plt.tight_layout()
plt.savefig("interpR2_vs_nparams_axial_log_affine_vs_plain.png", dpi=300, bbox_inches="tight")
plt.savefig("ann_nlayers_interp.svg", dpi=300, bbox_inches="tight")
plt.show()
