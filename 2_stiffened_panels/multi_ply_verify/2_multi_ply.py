...
ply_angles_deg = [0.0, 45.0, -45.0, 90.0]
ply_angles_deg = np.array(ply_angles_deg + ply_angles_deg[::-1])
# ply_angles_deg = np.array([0.0, 65.0, 65.0, 0.0]) # so sym laminate
...
# plate_SR = 300.0
plate_SR = 300.0 * 0.5**0.5 # SR adjustment
# plate_SR = 800.0
# plate_SR = 100.0
...
h_w = hw_mult * _h_w
AR = _AR * AR_mult
ply_angles_rad = np.deg2rad(ply_angles_deg)
b = 1.0
h = b / plate_SR
t_w = h_w / stiff_AR
a = b * AR
...
stiff_analysis = gen_mesh(comm, a, b, h, h_w, t_w, E1=E1, E2=E2, nu12=nu12, G=G, nstiff=nstiff, 
                          theta=np.mean(ply_angles_deg), strain_mult=strain_mult, nelems=nelems)
...
n_plies = ply_angles_deg.shape[0]
...
def elemCallBack(
    ...
    for iply in range(n_plies):
        ...
        # print(F"{ply_angles_deg[iply]}")
        # exit()
        util = mlb.CompositeMaterialUtility(E1, E2, nu12, G).rotate_ply(ply_angles_deg[iply])
    ...
    con = constitutive.CompositeShellConstitutive(
        ortho_layups,
        plate_thickness * ply_fractions,
        ply_angles_rad * 0.0, # since we've adjusted E1, E2 constants, etc.
        tOffset=0.0,
    )
...
if not os.path.exists("_multiply"):
    os.mkdir("_multiply")
SP = FEAAssembler.createStaticProblem(
    name="static"
)
SP.solve()
SP.writeSolution(outputDir="_multiply")
...
bucklingProb.writeSolution(outputDir="_multiply")
