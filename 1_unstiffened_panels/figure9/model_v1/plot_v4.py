...
# # try clipping the data at KMIN = 10? not working well
# nx, ny = KMIN.shape[0], KMIN.shape[1]
# for ix in range(nx):
#     for iy in range(ny):
#         if KMIN[ix,iy] > 10.0:
#             KMIN[ix,iy] = np.nan
KMIN_MASKED = np.ma.masked_where(KMIN >= 15, KMIN)

# plot the model curve
# Creating plot
# face_colors = cm.jet(KMIN / 10.0)
face_colors = cm.Grays(0.6 * KMIN / KMIN)
ax.plot_surface(
    DSTAR,
    AR,
    # KMIN,
...
ax.set_zticklabels(zticks, fontweight='bold')
...
# plt.show()
plt.savefig(f"3d-slender-{ibin}-v4.svg", dpi=400)
plt.savefig(f"3d-slender-{ibin}-v4.png", dpi=400)
plt.close("3d-GP")