...
class Fraction:
    ...
    def simplify(self):
        ...
        return Fraction(num=int(self.num / gcd), den=int(self.den / gcd))
    ...
    def __add__(self, obj):
        if isinstance(obj, float):
            ...
            # a/b + c/d = (ad + bc) / bd
            return Fraction(
                num=self.num * obj.den + self.den * obj.num, den=self.den * obj.den
            ).simplify
    ...
    def __mul__(self, obj):
        if isinstance(obj, float):
            ...
            # a/b * c/d = ac/bd
            return Fraction(num=self.num * obj.num, den=self.den * obj.den).simplify
    ...
    def __truediv__(self, obj):
        if isinstance(obj, Fraction):
            # a/b / (c/d) = ad/bc
            return Fraction(num=self.num * obj.den, den=self.den * obj.num).simplify


class SymbolGroup:
    @classmethod
    def from_letter(cls, letter, float=1.0):
        return SymbolGroup.from_symbols([Symbol(letter, float=float)])
    ...
    def __mul__(self, obj):
        if isinstance(obj, SymbolGroup):
            ...
            return SymbolGroup(
                float=self.float * obj, letters=self.letters, exponents=self.exponents
            ).simplify
...
class DualSin:
    ...
    @classmethod
    def copy(cls, obj):
        return cls(arg1=obj.arg1, arg2=obj.arg2, const=obj.const, coeff=obj.coeff)
    ...
    def __mul__(self, obj):
        ...
        # distributive property (with AddGroup)
            ...
            return AddGroup([self * _obj * obj.factor for _obj in obj.objs])
    ...
    def derivative(self, sym, order=1):
        ...
        assert sym in [
            "x1",
            "x2",
        ]  # for now as I don't want to multiply by add groups yet
...
class AddGroup:
    ...
    def simplify(self):
        ...
        new_group = AddGroup(
            [obj.simplify for obj in new_group.objs if not (obj.float == 0.0)],
        ...
        if common_factor is not None and not (common_factor == SymbolGroup.num(1.0)):
            new_group = AddGroup(
                objs=[obj / common_factor for obj in new_group.objs],
    ...
    def derivative(self, sym, order=1):
        ...
        first_deriv = AddGroup(
            [obj.derivative(sym, order) for obj in self.objs], factor=self.factor
        ) + AddGroup(self.objs, self.factor.derivative(sym, order))
        if order == 1:
            return first_deriv
        else:
            # recursive call for higher order derivatives
