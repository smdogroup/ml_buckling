...
parent_parser.add_argument("--kernel", type=int, default=1)
...
if kernel_option == 1:
    ...
    def kernel(xp, xq, theta):
        ...
        kernel2 = 1.0 + 0.5 * xp[2] * xq[2] + 0.1 * np.exp(-0.5 * d3 ** 2 / 9.0)
        # log(zeta) direction
        kernel3 = (1.0 + 0.2 * xp[3] * xq[3])**2 + 0.1 * np.exp(-0.5 * d2 ** 2 / 9.0)
    ...
    def kernel(xp, xq, theta):
        ...
        kernel1_2 = (
            0.02 #* np.exp(-0.5 * (xp[3] + xq[3]) / 1.0) # 0.2
            # * np.exp(-0.5 * (d1 ** 2 / (0.2 ** 2))
            * np.exp(-0.5 * (d1 ** 2 / 0.4 ** 2))
            * soft_relu(1.0 - xp[1]/3.0) * soft_relu(xp[1]/3.0)
            * soft_relu(1.0 - xq[1]/3.0) * soft_relu(xq[1]/3.0)
        )
        # log(gen eps) direction
        #  kernel2 = xp[2] * xq[2] + 0.1 * np.exp(-0.5 * d2 ** 2 / 9.0)
        kernel2 = 1.0 + 0.5 * xp[2] * xq[2] + 0.1 * np.exp(-0.5 * d3 ** 2 / 9.0)
